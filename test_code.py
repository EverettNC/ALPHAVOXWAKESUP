    ...
    try:
        ...
        try:
            ...
            if using_real:
                ...
                cv2.circle(
                    processed_img, (210, 200), 15, (0, 255, 0), 2
                )  # Left eye tracking
                cv2.circle(
                    processed_img, (410, 200), 15, (0, 255, 0), 2
                )  # Right eye tracking

                # Draw gaze direction
                cv2.arrowedLine(
                    processed_img, (210, 200), (250, 180), (0, 0, 255), 2
                )  # Left eye gaze
                cv2.arrowedLine(
                    processed_img, (410, 200), (450, 180), (0, 0, 255), 2
                )  # Right eye gaze
            ...
            cv2.imwrite(
                os.path.join(output_dir, "eye_test_processed.jpg"), processed_img
            )
            ...
            logger.info(
                "You can still run the application, but this test requires OpenCV"
            )
            return False
...
def test_behavior_capture():
    ...
    try:
        ...
        logger.info(
            f"Generated sample behavior observations at {output_dir}/sample_observations.json"
        )
        ...
        if behavior_capture:
            ...
            logger.info(
                f"Behavior tracking summary saved to {output_dir}/tracking_summary.json"
            )
    ...
    try:
        ...
        try:
            ...
            if engine:
                ...
                with open(
                    os.path.join(output_dir, "improvement_suggestions.json"), "w"
                ) as f:
                    json.dump(suggestions, f, indent=2)

                logger.info(
                    f"Saved improvement suggestions to {output_dir}/improvement_suggestions.json"
                )
            else:
            ...
            logger.info(
                f"Generated sample learning data at {output_dir}/sample_suggestions.json"
            )
    ...
    try:
        ...
        for gesture in test_gestures:
            ...
            if using_real:
                ...
                results.append({"gesture": gesture, "response": response})
            else:
        ...
        logger.info(
            f"Nonverbal test results saved to {output_dir}/nonverbal_test_results.json"
        )
        return True
...
def test_knowledge_base():
    ...
    try:
            ...
            if os.path.exists(file_path):
                ...
                with open(file_path, "r") as f:
                    ...
                    if isinstance(data, list):
                        ...
                        if len(data) > 0:
                            logger.info(
                                f"  - First item sample: {str(data[0])[:100]}..."
                            )
        ...
        logger.info(
            f"Knowledge test results saved to {output_dir}/knowledge_test_results.json"
        )
