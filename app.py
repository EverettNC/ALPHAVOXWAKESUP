...
def init_services():
    ...
    try:
        ...
        logging.warning(
            f"Real eye tracking not available: {str(e)}, using simulated service"
        )
    ...
    try:
        ...
        logging.warning(
            f"Real speech recognition not available: {str(e)}, using simulated service"
        )
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logging.error(
                    f"Detailed error registering learning routes: {route_error}"
                )
                logging.error(traceback.format_exc())
                ...
                try:
                    ...
                    logging.info(
                        f"Source of learning_routes: {inspect.getfile(learning_routes)}"
                    )
                except Exception as import_error:
                    logging.error(
                        f"Error importing learning_routes directly: {import_error}"
                    )
                    logging.error(traceback.format_exc())
            ...
            def learning_hub_fallback():
                ...
                try:
                    ...
                    stats = {
                        "topics_explored": len(
                            base_stats.get("topic_progress", {}).keys()
                        ),
                        ...
                        "topic_mastery": {
                            k: int(v * 100)
                            for k, v in base_stats.get("topic_progress", {}).items()
                        },
                        "recent_activities": [],
                    }
                    ...
                    return render_template(
                        "learning/hub.html", topics=topics, stats=stats
                    )
                except Exception as e:
...
def save_interaction(text, intent, confidence):
    ...
    try:
        ...
        db_interaction = UserInteraction(
            user_id=user_id, text=text, intent=intent, confidence=confidence
        )
...
def text_to_speech(text, emotion=None, emotion_tier=None, voice_id="us_male"):
    ...
    try:
        from attached_assets.advanced_tts_service import (
        ...
        filepath = text_to_speech_with_emotion(
            text=text, emotion=emotion, emotion_tier=emotion_tier, voice_id=voice_id
        )
        ...
        # Log speech details
        logging.debug(
            f"Speaking with emotion: {emotion}, tier: {emotion_tier}, rate: {rate}"
        )
        ...
        text_hash = hashlib.md5(
            f"{text}_{emotion}_{emotion_tier}_{rate}".encode()
        ).hexdigest()
...
def demo_male_voice():
    ...
    try:
        ...
        speech_file = text_to_speech(
            text=text, emotion="neutral", emotion_tier="moderate", voice_id="us_male"
        )
...
def generate_speech_api():
    ...
    # Log the request
    logging.info(
        f"Generating speech: voice={voice_id}, emotion={emotion}, tier={emotion_tier}"
    )
    ...
    try:
        ...
        return jsonify({"status": "success", "speech_url": speech_url})
    except Exception as e:
        logging.error(f"Error generating speech: {str(e)}")
        logging.error(traceback.format_exc())
...
def get_audio_devices():
    ...
    devices = [
        {"id": "sim-1", "name": "Default Microphone", "channels": 1, "default": True}
    ]
...
def process_input():
    ...
    try:
        ...
        message = interaction.get(
            "message", f"I understand you're saying: {input_text}"
        )
        ...
        logging.info(
            f"Processed text with AlphaVox NLU: '{input_text}' → {intent} ({confidence:.2f})"
        )
...
def process_input_basic(input_text):
    ...
    if nonverbal_engine:
        ...
        if any(
            word in input_text.lower() for word in ["help", "need", "please", "urgent"]
        ):
            intent = "help"
            expression = "urgent"
            emotion_tier = "strong"
            ...
            word in input_text.lower() for word in ["happy", "glad", "thank", "good"]
        ):
            intent = "express_joy"
            expression = "positive"
            emotion_tier = "moderate"
            ...
            word in input_text.lower() for word in ["sad", "upset", "sorry", "bad"]
        ):
            ...
            for word in ["question", "?", "ask", "why", "how", "what", "when", "where"]
        ):
            intent = "ask_question"
...
def speak_greeting():
    ...
    try:
        ...
        return jsonify(
            {"status": "success", "message": message, "speech_url": speech_url}
        )
    except Exception as e:
...
def speak(gesture):
    ...
    try:
        ...
        message = interaction.get(
            "message", f"I'm communicating with a {gesture} gesture."
        )
        intent = interaction.get("intent", "communicate")
        ...
        # Log the processing
        logging.info(
            f"Processed gesture with AlphaVox NLU: '{gesture}' → {intent} ({confidence:.2f})"
        )
        ...
        message, intent, confidence, expression, emotion_tier = process_gesture_basic(
            gesture
        )
        advanced_ai = False
...
def process_gesture_basic(gesture):
    ...
    if nonverbal_engine:
        ...
        logging.debug(
            f"Gesture {gesture} classified as {intent} with emotion {expression}, tier: {emotion_tier}"
        )
    else:
        intent = "communicate"
        confidence = 0.7
...
def start_behavior_capture():
    ...
    if behavior_capture:
        ...
        return (
            jsonify(
                {"status": "error", "message": "Behavior capture system not available"}
            ),
            500,
        )
...
def stop_behavior_capture():
    ...
    if not behavior_capture:
        return jsonify(
            {"status": "error", "message": "Behavior capture not initialized"}
        )

    behavior_capture.stop_tracking()

    return jsonify(
        {"status": "success", "tracking": False, "message": "Behavior capture stopped"}
    )
...
def get_behavior_status():
    ...
    return jsonify({"status": "success", "tracking_status": status})
...
def get_behavior_observations():
    ...
    try:
        ...
        observations_file = os.path.join(
            "data", "behavior_patterns", "behavior_observations.json"
        )

        if not os.path.exists(observations_file):
            return jsonify({"status": "success", "observations": []})
    ...
    try:
        ...
        if symbol_name in default_messages:
            message = default_messages[
                symbol_name
            ]  # Prioritize defaults over AI-generated
        else:
        ...
        # Log the processing
        logging.info(
            f"Processed symbol with AlphaVox NLU: '{symbol_name}' → {intent} ({confidence:.2f})"
        )
        ...
        message, intent, confidence, expression, emotion_tier = process_symbol_basic(
            symbol_name
        )
        advanced_ai = False
...
def process_symbol_basic(symbol_name):
    ...
    if (
        nonverbal_engine
        and hasattr(nonverbal_engine, "symbol_map")
        and symbol_name in nonverbal_engine.symbol_map
    ):
        result = nonverbal_engine.symbol_map[symbol_name]
        intent = result.get("intent", "communicate")
        confidence = result.get("confidence", 0.7)

        message = symbol_messages.get(
...
def start_ai():
    ...
    if ai_running:
        return jsonify(
            {"status": "already_running", "message": "AI assistance is already active"}
        )
    ...
    return jsonify({"status": "started", "message": "AI assistance activated"})
...
def stop_ai():
    ...
    return jsonify({"status": "stopped", "message": "AI assistance deactivated"})
...
def ai_control():
    ...
    research_status = {"last_update": None, "articles_count": 0, "insights_count": 0}
    ...
    stats = {
        "interactions_count": (
            sum(len(stats) for stats in improvements.values()) if improvements else 0
        ),
        "pending_modifications_count": len(
            [m for m in recent_modifications if m["status"] == "pending"]
        ),
        "applied_modifications_count": len(
            [m for m in recent_modifications if m["status"] == "applied"]
        ),
        ...
        return jsonify(
            {
                ...
                "message": (
                    "Learning process started" if result else "Learning already active"
                ),
            }
        )
    except Exception as e:
        ...
        return jsonify(
            {
                ...
                "message": (
                    "Learning process stopped"
                    if result
                    else "Learning already inactive"
                ),
            }
        )
    except Exception as e:
        ...
        return jsonify(
            {
                "status": "success" if result else "already_running",
                "message": (
                    "Auto-modification mode started"
                    if result
                    else "Auto-modification already active"
                ),
            }
        )
    except Exception as e:
        ...
        return jsonify(
            {
                "status": "success" if result else "already_stopped",
                "message": (
                    "Auto-modification mode stopped"
                    if result
                    else "Auto-modification already inactive"
                ),
            }
        )
    except Exception as e:
...
def queue_modification():
    ...
    try:
        ...
        return jsonify(
            {"status": "success", "message": "Modification queued successfully"}
        )
    except Exception as e:
...
def get_improvements():
    ...
    try:
                    ...
                    improvements.append(
                        {
                            ...
                            "confidence": (
                                round(
                                    item_stats.get("confidence_sum", 0)
                                    / item_stats.get("count", 1)
                                    * 100
                                )
                                if item_stats.get("count", 0) > 0
                                else 0
                            ),
        ...
        return jsonify({"status": "success", "improvements": improvements})
    except Exception as e:
        logging.error(f"Error fetching improvements: {str(e)}")
        logging.error(traceback.format_exc())
        return jsonify(
            {"status": "error", "message": f"Error: {str(e)}", "improvements": []}
        )
        ...
        for section in stats.values():
            if isinstance(section, dict):
                interaction_count += sum(
                    item.get("count", 0) for item in section.values()
                )
        ...
        return jsonify(
            {
                ...
                "last_optimization": (
                    learning_engine.last_optimization.strftime("%Y-%m-%d %H:%M:%S")
                    if hasattr(learning_engine, "last_optimization")
                    else None
                ),
                "learning_actions": learning_actions,
                "learning_active": learning_engine.learning_active,
