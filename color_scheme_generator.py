...
if OPENAI_API_KEY:
    ...
    logging.warning(
        "OpenAI API key not found. AI color generation will not be available."
    )
...
class ColorSchemeGenerator:
    ...
    def save_user_color_scheme(
        ...
        if user_id not in self.color_profiles:
            ...
            if "scheme" in self.color_profiles[user_id]:
                ...
                self.color_profiles[user_id]["history"] = self.color_profiles[user_id][
                    "history"
                ][-5:]
    ...
    def _generate_algorithmic_color_scheme(
        ...
        if brightness == "bright":
            # Brighten background
            base_scheme["background"] = self._lighten_color(
                base_scheme["background"], 0.3
            )
            base_scheme["surface"] = self._lighten_color(base_scheme["surface"], 0.3)
        elif brightness == "dark":
            # Darken background
            base_scheme["background"] = self._darken_color(
                base_scheme["background"], 0.1
            )
        ...
        for key in color_keys:
            if key in scheme:
                try:
                    ...
                    enhanced_scheme[f"{key}_rgb"] = fallbacks.get(
                        f"{key}_rgb", "0, 0, 0"
                    )
