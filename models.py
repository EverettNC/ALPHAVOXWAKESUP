...
class CaregiverNote(db.Model):
    ...
    user = db.relationship(
        "User", backref=db.backref("caregiver_notes", lazy="dynamic")
    )
...
class CommunicationProfile(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False)
    primary_mode = db.Column(
        db.String(64), nullable=False
    )  # 'gesture', 'symbol', 'text', etc.
    ...
    user = db.relationship(
        "User", backref=db.backref("communication_profiles", lazy="dynamic")
    )
    ...
    def get_latest_profile(cls, user_id):
        ...
        return (
            cls.query.filter_by(user_id=user_id).order_by(cls.updated_at.desc()).first()
        )
...
class SystemSuggestion(db.Model):
    ...
    user = db.relationship(
        "User", backref=db.backref("system_suggestions", lazy="dynamic")
    )
...
class LearningMilestone(db.Model):
    ...
    user = db.relationship(
        "User", backref=db.backref("learning_milestones", lazy="dynamic")
    )
...
class LearningTemplate(db.Model):
    ...
    description = db.Column(db.Text, nullable=False)
    focus_area = db.Column(
        db.String(64), nullable=False
    )  # 'symbols', 'gestures', 'intent', etc.
    ...
    user = db.relationship(
        "User", backref=db.backref("learning_templates", lazy="dynamic")
    )
    ...
    @classmethod
    def create_template(
        cls, user_id, name, description, focus_area, parameters, activate=False
    ):
        """Create a new learning template"""
        # Deactivate other templates if needed
        if activate:
            active_templates = cls.query.filter_by(
    ...
    def activate_template(cls, template_id):
        ...
        active_templates = cls.query.filter_by(
            user_id=template.user_id, is_active=True
        ).all()
...
class LearningSession(db.Model):
    ...
    template_id = db.Column(
        db.Integer, db.ForeignKey("learning_template.id"), nullable=True
    )
    notes = db.Column(db.Text, nullable=True)

    user = db.relationship(
        "User", backref=db.backref("learning_sessions", lazy="dynamic")
    )
    template = db.relationship(
        "LearningTemplate", backref=db.backref("sessions", lazy="dynamic")
    )
...
class SkillLevel(db.Model):
    ...
    def update_skill(cls, user_id, skill_type, change=1):
        ...
        if skill:
            ...
            # Create new skill
            skill = cls(
                user_id=user_id, skill_type=skill_type, level=min(100, max(0, change))
            )
...
class RecognitionFeedback(db.Model):
    ...
    interaction_id = db.Column(
        db.Integer, db.ForeignKey("user_interaction.id"), nullable=False
    )
    ...
    user = db.relationship(
        "User", backref=db.backref("recognition_feedback", lazy="dynamic")
    )
    interaction = db.relationship(
        "UserInteraction", backref=db.backref("feedback", lazy="dynamic")
    )
    ...
    @classmethod
    def add_feedback(
        cls, user_id, interaction_id, feedback_type, correct_intent=None, notes=None
    ):
