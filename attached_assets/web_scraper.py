...
def get_website_text_content(url: str) -> str:
    ...
    try:
        ...
        if text:
            logger.info(
                f"Successfully extracted content from {url} ({len(text)} characters)"
            )
            return text
        else:
...
def get_links_from_search(
    query: str, search_type: str = "general", max_results: int = 5
) -> list:
    """
    ...
    """
    try:
        search_url = SEARCH_URLS.get(
            search_type, SEARCH_URLS["general"]
...
def get_content_from_trusted_sources(topic: str, max_sources: int = 3) -> list:
    ...
    try:
        ...
        logger.info(
            f"Found content from {len(results)} trusted sources for topic: {topic}"
        )
        return results
...
if __name__ == "__main__":
    ...
    for topic in test_topics:
        ...
        print(
            f"Education result for '{topic}': {result['sources_count']} sources found"
        )
