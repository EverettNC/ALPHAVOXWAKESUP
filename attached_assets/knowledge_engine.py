    ...
    def get_facts_by_topic(self, topic: str, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        if len(indices) > limit:
            ...
            high_conf_candidates = [
                i for i in indices if self.facts[i]["confidence"] > 0.7
            ]
            ...
            if len(high_conf_candidates) > limit:
                ...
                remaining_candidates = [
                    i for i in indices if i not in high_conf_candidates
                ]
        ...
        for idx in selected_indices:
            self.facts[idx]["times_accessed"] = (
                self.facts[idx].get("times_accessed", 0) + 1
            )
...
class WebCrawler:
    ...
    def _generate_fact_for_topic(self, topic: str) -> str:
        ...
        topic_facts = facts_by_topic.get(
            topic,
            [
    ...
    def __init__(self):
        ...
        self.metrics = {
            ...
            "topics_explored": len(
                set(
                    sum(
                        [fact.get("topics", []) for fact in self.fact_manager.facts], []
                    )
                )
            ),
    ...
    def process_expertise_query(self, query: str) -> Dict[str, Any]:
        ...
        for topic in NONVERBAL_TOPICS:
            if topic.lower() in query_lower:
                matched_topic = topic
                matching_facts.extend(
                    self.fact_manager.get_facts_by_topic(topic, limit=3)
                )
                break
        ...
        if matching_facts:
            ...
            response_text = (
                "I'm still learning about that topic. Here's a related fact:"
            )
