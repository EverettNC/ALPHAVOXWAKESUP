...
def view_topic(topic_name):
    ...
    facts_learned = sum(1 for fact_id, mastery in fact_mastery.items() 
...
def admin_add_fact():
    ...
    # Add the fact
    try:
        fact = learning_journey.add_fact(
            topic=topic,
            content=content,
            source=source
        )
...
def admin_connect_concepts():
    ...
    if len(concepts) < 2:
        return jsonify({
            "status": "error",
            "message": "Need at least 2 concepts to connect"
        }), 400
    ...
    learning_journey.log_event(
        user_id=user_id,
        event_type="concept_connected",
        details={"concepts": concepts}
    )
    
    return jsonify({
        "status": "success",
        "message": "Concepts connected successfully"
    })
