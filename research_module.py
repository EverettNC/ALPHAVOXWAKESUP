...
class AlphaVoxResearchModule:
                ...
                for link in soup.find_all("a", href=True)[:10]:  # Limit to 10 per term
                    ...
                    try:
                        article_response = requests.get(
                            href, headers=headers, timeout=10
                        )
                        ...
                        content = " ".join(
                            p.text for p in article_soup.find_all("p")[:5]
                        )  # First 5 paragraphs
    ...
    def integrate_insights(
        self, insights: List[Dict], nlc: Optional[object] = None
    ) -> Dict:
        ...
        try:
            ...
            for insight in insights:
                ...
                if any(
                    phrase in key_phrases
                    for phrase in ["therapy", "intervention", "esdm"]
                ):
                    updates["therapy_recommendations"].append(
                        {
                            "therapy": (
                                key_phrases[0] if key_phrases else "general_therapy"
                            ),
                            "description": summary[:100],
                            "source": (
                                insight["articles"][0]["url"]
                                if insight["articles"]
                                else ""
                            ),
                        }
            ...
            if nlc:
                try:
                    for strategy in updates["new_strategies"]:
                        nlc.intent_weights[strategy["type"]] = (
                            nlc.intent_weights.get(strategy["type"], 0.0) + 0.1
                        )
                    for intent in updates["updated_intents"]:
                        nlc.intent_weights[intent["intent"]] = intent["weight"]
                    logger.info(
                        "Integrated research insights into Neural Learning Core"
                    )
                except Exception as e:
