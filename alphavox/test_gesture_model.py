...
def test_gestures(model):
    ...
    for name, features in test_gestures.items():
        try:
            ...
            print(
                f"{color}Testing '{name}' gesture: Predicted '{prediction}' with {confidence:.1f}% confidence - {result}\033[0m"
            )
    ...
    try:
        ...
        confidence = max(model.predict_proba(features_array)[0]) * 100
        print(
            f"Custom gesture [0.6, 0.7, 140, 50]: Predicted '{prediction}' with {confidence:.1f}% confidence"
        )
    except Exception as e:
