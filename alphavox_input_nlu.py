...
class AlphaVoxInputProcessor:
    ...
    def load_context(self):
        ...
        if os.path.exists(CONTEXT_FILE):
            try:
                ...
                logger.info(
                    f"Loaded context window with {len(self.context_window)} users"
                )
            except Exception as e:
    ...
    def save_context(self):
        ...
        with self.lock:
            try:
                ...
                logger.info(
                    f"Saved context window for {len(self.context_window)} users"
                )
            except Exception as e:
    ...
    def update_from_research(self):
        ...
        try:
            ...
            for strategy in research_updates.get("updates_applied", {}).get(
                "new_strategies", []
            ):
                if (
            ...
            for intent in research_updates.get("updates_applied", {}).get(
                "updated_intents", []
            ):
    ...
    def process_interaction(
        self, interaction: Dict[str, Any], user_id: str
    ) -> Dict[str, Any]:
    ...
    def _process_text(self, interaction: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            interaction["message"] = (
                text if intent == "communicate" else f"I want to say: {text}"
            )
            interaction["emotion"] = emotion
            interaction["confidence"] = confidence

            logger.info(
                f"Processed text: {text} -> {intent} (confidence: {confidence:.2f})"
            )
            return interaction
        except Exception as e:
    ...
    def _add_context(self, interaction: Dict[str, Any], user_id: str) -> Dict[str, Any]:
        ...
        with self.lock:
            if user_id in self.context_window:
                ...
                recent_count = sum(
                    1 for ts in interactions if (now - ts).total_seconds() < 3600
                )
    ...
    def _get_research_context(self) -> Dict[str, Any]:
        ...
        try:
            ...
            therapy_recommendations = research_updates.get(
